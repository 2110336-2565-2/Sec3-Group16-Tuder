...
func (rq *ReviewQuery) QueryCourse() *CourseQuery {
	query := (&CourseClient{config: rq.config}).Query()
...
func (rq *ReviewQuery) QueryStudent() *StudentQuery {
...
func (rq *ReviewQuery) Clone() *ReviewQuery {
	...
	return &ReviewQuery{
		...
		withCourse:  rq.withCourse.Clone(),
...
func (rq *ReviewQuery) WithCourse(opts ...func(*CourseQuery)) *ReviewQuery {
	query := (&CourseClient{config: rq.config}).Query()
...
func (rq *ReviewQuery) WithStudent(opts ...func(*StudentQuery)) *ReviewQuery {
	query := (&StudentClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *ReviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Review, error) {
	...
	if query := rq.withCourse; query != nil {
		if err := rq.loadCourse(ctx, query, nodes,
			func(n *Review) { n.Edges.Course = []*Course{} },
			func(n *Review, e *Course) { n.Edges.Course = append(n.Edges.Course, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withStudent; query != nil {
		if err := rq.loadStudent(ctx, query, nodes,
			func(n *Review) { n.Edges.Student = []*Student{} },
			func(n *Review, e *Student) { n.Edges.Student = append(n.Edges.Student, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReviewQuery) loadCourse(ctx context.Context, query *CourseQuery, nodes []*Review, init func(*Review), assign func(*Review, *Course)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(review.CourseTable)
		s.Join(joinT).On(s.C(course.FieldID), joinT.C(review.CoursePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(review.CoursePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(review.CoursePrimaryKey[1]))
...
func (rq *ReviewQuery) loadStudent(ctx context.Context, query *StudentQuery, nodes []*Review, init func(*Review), assign func(*Review, *Student)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(review.StudentTable)
		s.Join(joinT).On(s.C(student.FieldID), joinT.C(review.StudentPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(review.StudentPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(review.StudentPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
