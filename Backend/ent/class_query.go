...
func (cq *ClassQuery) loadMatch(ctx context.Context, query *MatchQuery, nodes []*Class, init func(*Class), assign func(*Class, *Match)) error {
	...
	query.Where(predicate.Match(func(s *sql.Selector) {
		s.Where(sql.InValues(class.MatchColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.class_match
		if fk == nil {
			return fmt.Errorf(`foreign-key "class_match" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "class_match" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
