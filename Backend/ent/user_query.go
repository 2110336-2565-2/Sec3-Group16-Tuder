...
func (uq *UserQuery) QueryPaymentHistory() *PaymentHistoryQuery {
	query := (&PaymentHistoryClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryCancelRequest() *CancelRequestQuery {
	query := (&CancelRequestClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithPaymentHistory(opts ...func(*PaymentHistoryQuery)) *UserQuery {
	query := (&PaymentHistoryClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithCancelRequest(opts ...func(*CancelRequestQuery)) *UserQuery {
	query := (&CancelRequestClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withCancelRequest = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPaymentHistory; query != nil {
	...
	if query := uq.withCancelRequest; query != nil {
		if err := uq.loadCancelRequest(ctx, query, nodes,
			func(n *User) { n.Edges.CancelRequest = []*CancelRequest{} },
			func(n *User, e *CancelRequest) { n.Edges.CancelRequest = append(n.Edges.CancelRequest, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadPayment(ctx context.Context, query *PaymentQuery, nodes []*User, init func(*User), assign func(*User, *Payment)) error {
	...
	query.Where(predicate.Payment(func(s *sql.Selector) {
		s.Where(sql.InValues(user.PaymentColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_payment" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadPaymentHistory(ctx context.Context, query *PaymentHistoryQuery, nodes []*User, init func(*User), assign func(*User, *PaymentHistory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.PaymentHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(user.PaymentHistoryColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_payment_history
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_payment_history" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) loadCancelRequest(ctx context.Context, query *CancelRequestQuery, nodes []*User, init func(*User), assign func(*User, *CancelRequest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.CancelRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(user.CancelRequestColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_cancel_request
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_cancel_request" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_cancel_request" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
